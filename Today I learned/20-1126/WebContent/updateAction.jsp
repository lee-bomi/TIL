...
<body>
	<%
		...
		if(!userID.equals(lbm.getUserID())){
			...
			if(request.getParameter("lbmTitle") == null || request.getParameter("lbmContent") == null ||
				request.getParameter("lbmTitle").equals("") || request.getParameter("lbmContent").equals("")){
				PrintWriter script = response.getWriter();
				script.println("<script>");
				...
				int result = lbmDAO.update(lbmID, request.getParameter("lbmTitle"), request.getParameter("lbmContent"));
				if(result == -1){
					PrintWriter script = response.getWriter();
					script.println("<script>");
					script.println("alert('글 수정에 실패했습니다(db오류)')");
					script.println("history.back()");
					script.println("</script>");
				}
				else {
